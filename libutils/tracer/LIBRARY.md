...
| `host-tests/` | host 单测（`test_tracer.c` 纯逻辑 + `test_parser.py` 离线解析），`-DTRACER_BUILD_TESTS=ON` 启用 |
| `LICENSE` | MIT 许可证 |
| `README.md` | 本文档 |
...
- 块推送宏：`TRACER_LOG_SINK_CHUNK_SIZE`（默认 `128u`，一块字节数，可 `-D` 覆盖；调小可让 sink 更及时、
 ...
 - host 单测 `host-tests/test_tracer_log.c`（格式/运行期分级/合一 ring/超长不截断/drain/崩溃 record 自动含最近
 日志）与 `host-tests/test_tracer_log_sink.c`（weak sink 强覆盖、过滤 不回调、长行分块+残余 flush）随 CI 运行。
