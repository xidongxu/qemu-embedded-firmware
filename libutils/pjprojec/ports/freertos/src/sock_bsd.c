...
#else
/* lwIP does not define MSG_DONTROUTE */
...
{
    /* lwIP has no gethostname(); return a fixed board name. */
    static char buf[PJ_MAX_HOSTNAME] = "mps2-an505";
...
PJ_DEF(pj_status_t) pj_sock_close(pj_sock_t sock)
{
    ...
    rc = closesocket(sock);
#else
    /* lwIP: use closesocket() which maps to lwip_close(); newlib's close()
     * is a stub that returns -1. */
    rc = closesocket(sock);
#endif
