...
static void lwip_os_task_entry(void *parameters) {
    ...
    /* Dual-QEMU call / phone mode: keep the network up (tcpip + eth_rx) but
     * skip the HTTP-to-internet / ping self-tests.  A/B test (2026-08-20)
     * showed they WORSEN the slirp forwarding loss (with them ON, 2/6
     * directions dropped 64/142 of 200 frames vs near-zero with them OFF),
     * and the 64 KB HTTP bursts flood the guest RX, dropping SIP responses
     * (180/BYE) -> slow ring and missed remote hang-up in the phone app. */
    while (true) {
